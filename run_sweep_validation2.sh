...
ds_args() {
  ...
  case "$ds" in
    adult)
      ...
      ;;
    sparse_adult)
      echo "--dataset sparse_adult --sens_keys sex,race,age,marital-status --sens_thresh 0.5 --data_dir ../data/raw/"
      ;;
    communities)
...
# ===== 스윕 그리드 =====
# DR
# DR_LAMS=(0.00 0.05 0.10 0.50 1.00 2.00 5.00 10.00 50.0 100.0 500.0 1000.0 5000.0 10000.0)
DR_LAMS=(0.000001 0.000003 0.000005 0.00001 0.00003 0.00005 0.001 0.003 0.005 0.1 0.3 0.5 30.0 50.0 300.0 500.0 1000.0)
# DR_LAMS=(5.00)
# DR_NLOWS_FRAC=(0.001 0.002 0.003 0.005)
DR_NLOWS_FRAC=(0.001 0.003 0.005 0.01 0.03 0.05 0.1 0.3)
...
build_dr_subgroup_subset_3q_cmds() {
  ...
  for ds in sparse_adult dutch communities civilcomments; do
    extra=$(ds_args "$ds")
      for nlow_frac in "${DR_NLOWS_FRAC[@]}"; do
        for lam in "${DR_LAMS[@]}"; do
          ...
          enqueue "$(gpu_prefix) $PY run_experiments_3q.py $extra --method dr_subgroup_subset_random \
            ...
            --save_dir \"../0917val_553_$SAVE_DIR/$nlow_frac/dr_$ds\""
        done
      done
    done
  done
    ...
    extra=$(ds_args "$ds")
      for c in "${GF_CS[@]}"; do
        ...
        enqueue "$(gpu_prefix) $PY run_experiments_3q.py $extra --method gerryfair \
          ...
          --save_dir \"../0916val_4_$SAVE_DIR/gerryfair_$ds\""
      done
    done
  done
}
    ...
    extra=$(ds_args "$ds")
      ...
      for a in "${SEQ_ALPHAS[@]}"; do
        ...
        enqueue "$(gpu_prefix) $PY run_experiments_3q.py $extra --method sequential \
            ...
            --save_dir \"../0916val_4_$SAVE_DIR/sequential_$ds\""
        done
    done
  done
}
  ...
  for ds in adult; do
    extra=$(ds_args "$ds")
      for lam in "${REG_LAMS[@]}"; do
        for seed in "${SEED_ARR[@]}"; do
        enqueue "$(gpu_prefix) $PY run_experiments_3q.py $extra --method reg --af_max_order 1 \
          --mf_lambda $lam --mf_base mlp \
          --x_sensitive $X_SENSITIVE --seed $seed --sparse_n_groups 5 \
          --save_dir \"../0916val_5_$SAVE_DIR/reg_$ds\""
      done
    done
  done
}
